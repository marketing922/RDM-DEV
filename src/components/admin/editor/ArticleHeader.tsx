...
const ArticleHeader: React.FC<ViewDescriptionServerProps> = async (props) => {
  ...
  const title: string = doc.title || doc.name || 'Sans titre'

  const excerpt: string | undefined =
    typeof doc.excerpt === 'string' && doc.excerpt.trim().length > 0
      ? truncate(doc.excerpt.trim(), 100)
      : undefined

  const timeSource: string | undefined = doc.publishedAt || doc.updatedAt
  const relativeTime = formatRelative(timeSource)
