...
export function InstitBreadcrumb({
  ...
  return (
    <nav
      ...
      className="max-w-[1040px] mx-auto px-6 md:px-10 pt-5 font-sans text-[12px] text-rm-inkSoft"
    >
      {crumbs.map((c, i) => (
...
export function InstitHeader({
  ...
  return (
    <Reveal>
      <header className="bg-rm-cream border-b border-rm-rule pt-16 md:pt-[72px] pb-12 md:pb-14">
        <div className="max-w-[1040px] mx-auto px-6 md:px-10">
          ...
          <h1 className="font-display font-normal text-rm-teal m-0 leading-[0.9] tracking-[-0.025em] text-[56px] sm:text-[68px] md:text-[84px]">
            {title}
            {sub && (
              <em className="block italic text-rm-burgundy mt-2 text-[38px] sm:text-[46px] md:text-[52px] leading-[0.95]">
                {sub}
              </em>
            )}
          </h1>
          {intro && (
            <p className="font-serif text-[17px] md:text-[20px] leading-[1.55] text-rm-inkSoft mt-6 md:mt-7 max-w-[720px]">
              {intro}
            </p>
          )}
        </div>
...
export function InstitPage({
  ...
  return (
    <main className="min-h-screen bg-rm-cream text-rm-ink font-sans">
      ...
      <div
        className="mx-auto px-6 md:px-10 py-16 md:py-20 pb-24"
        style={{ maxWidth }}
      >
        {children}
      </div>
